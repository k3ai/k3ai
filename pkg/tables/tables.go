...
func List(appsResults []string,infraResults []string,bundlesResults []string,commsResults[]string) {
	...
	if len(appsResults) >0 {
		...
		for i:=0; i < len(appsResults)-1;i+= limit{
			...
			t.AppendRow(table.Row{strings.ToUpper(batch[0]),batch[1],batch[2],batch[3],batch[4],"Available"})
			t.AppendSeparator()
		}
		t.Render()
	}
